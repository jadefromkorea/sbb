...
public class SecurityConfig {
    /*
    @PreAuthorize 애너테이션이 동작할 수 있도록 => @EnableMethodSecurity(prePostEnabled = true)
    SecurityConfig에 적용한 @EnableMethodSecurity 애너테이션의 prePostEnabled = true는
    QuestionController와 AnswerController에서 로그인 여부를 판별할 때 사용한 @PreAuthorize 애너테이션을 사용하기 위해 반드시 필요한 설정이다.
     */
    @Bean
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                // .formLogin 메서드는 스프링 시큐리티의 로그인 설정을 담당하는 부분으로, 설정 내용은 로그인 페이지의 URL은 /user/login이고 로그인 성공 시에 이동할 페이지는 루트 URL(/)임을 의미한다.
